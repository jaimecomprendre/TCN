...
num_epochs_gan = 1000 #@param {type:"slider", min:0, max:10000, step:1}
...
ms1 = int(num_epochs_gan * 0.8)  
ms2 = int(num_epochs_gan * 0.95)  
milestones = [ms1, ms2]

# this is only for the progress 
pbar = tqdm(range(num_epochs_gan))
...
for epoch in pbar:
    ...
    #base rave loss
    # discriminator_loss = torch.relu(1 - real_predictions) + torch.relu(1+ fake_predictions)
    #without the +1
